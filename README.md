...
自動でClaudeに指示し、`/clear`（推奨・トークン消費ゼロ）または auto compact 後のコンテキストに
自動で再注入します。
...
3. Claudeが `.claude-handoff/<session_id>/current.md` に引き継ぎ資料を作成し、
   ユーザーへ **`/clear` の実行を推奨**（トークン消費ゼロ。Remote Control中はモバイル/Webからも実行可）
4. `/clear`（または放置してauto compact）後、SessionStartフックが検証済みの引き継ぎ資料+
   git状態+直近のユーザーメッセージを新しいコンテキストへ自動注入 → そのまま作業続行

`/clear` を推奨する理由: `/compact` は会話全体を送って要約を生成する大きなAPIリクエストで
課金/usage limitを消費しますが、`/clear` は何も消費しません。auto compactは
「`/clear` し忘れた場合の安全網」として働きます（best-effort。既知の限界1参照）。
